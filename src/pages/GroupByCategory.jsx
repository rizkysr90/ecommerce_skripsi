...
function GroupByCategory() {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                setIsLoading(true)
                const res = await axios.get(`${process.env.REACT_APP_API_HOST}/products?c=${categoryId}&active=true&page=${page}`)
            ...
            <div className='pt-20 bg-base-200'>
                ...
                }
                {
                    ...
                    <div className='mx-3 md:mx-14'>
                        <div className='ml-2'>Hasil pencarian untuk produk kategori '<span className='text-secondary font-bold'>
                            {`${categoryName}`}</span>'
                        </div>
