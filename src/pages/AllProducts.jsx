...
function AllProducts() {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                setIsLoading(true)
                const res = await axios.get(`${process.env.REACT_APP_API_HOST}/products?active=true&page=${page}`)
                ...
                }
                {
                    ...
                    <div className='mx-3 md:mx-14'>
                        <div className='flex justify-between'>
                            <div className='ml-2 font-bold'>Semua Produk
                            </div>
                            <Pagination
                            ...
                            products.length !== 0 ? products.map((data) => {
                                return (
                                    <Link to={`/products/${data?.id}`}
                                        className="card card-compact basis-5/12 grow sm:grow-0 sm:basis-[22%] lg:basis-[15%]  mx-2 mb-4 bg-base-100  rounded" 
                                        key={data.id}>
                                        <figure>
