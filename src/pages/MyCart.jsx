...
export default function Register() {
  ...
  const fetchCart = async (url) =>
    await axios.get(url).then((res) => res.data.data.Products);
  const { data: cart, isLoading } = useSWR(
    `${process.env.REACT_APP_API_HOST}/carts`,
    fetchCart
  );
  ...
  const removeProduct = async (id) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_HOST}/carts/remove/products/${id}`
      );
    } catch (error) {}
  };
  const changeQty = async (id) => {
    ...
    try {
      await axios.put(
        `${process.env.REACT_APP_API_HOST}/carts/update/products`,
        payload
      );
    } catch (error) {}
  };
        ...
        {isLoading ? (
          ...
          <div className="mx-3 md:mx-20">
                      ...
                      <div className="flex mb-3">
                        ...
                        <div className="ml-3 grow flex flex-col">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="modal modal-bottom sm:modal-middle">
                              <div className="modal-box">
                                <h3 className="font-bold text-lg">
                                  Konfirmasi {data?.name}
                                </h3>
                                <p className="py-4">
                                  Yakin ingin menghapus produk ini dari
                                  keranjang?
                                </p>
                            ...
                            <label
                              className="btn btn-sm normal-case btn-error text-base-100"
            ...
            <div className="flex justify-end mt-4">
              {product?.length < 1 ? (
                <div className="btn btn-disabled w-full md:w-auto normal-case text-base-100">
                  Checkout
                </div>
              ) : (
                <div
                  className="btn btn-secondary w-full md:w-auto normal-case text-base-100"
                  onClick={() => {
