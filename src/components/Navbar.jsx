...
export const Navbar = () => {
  ...
  const { data: categories } = useSWR(
    `${process.env.REACT_APP_API_HOST}/productCategories`,
    fetchCategories
  );
  const handleLogout = () => {
    dispatch(logoutUser());
    ...
    <>
      <div className="navbar bg-primary fixed top-0 z-50 py-2 px-1  lg:px-20 ">
        <div className="navbar-start w-auto md:w-1/2">
          <Link
            to="/"
            className="btn hidden md:flex btn-secondary normal-case "
          >
            Rizki Plastik
          </Link>
        ...
        <div className="navbar-end  w-auto md:w-1/2">
          {user ? (
              ...
              <div className="dropdown hidden md:flex dropdown-end">
                ...
                >
                  <Link
                    ...
                    className="btn normal-case btn-ghost text-neutral-content"
                  >
                    Alamat Pengiriman
                  </Link>
                  <Link
                    to="/customers/myorders"
                    className="btn normal-case btn-ghost text-neutral-content"
                  >
                    Pesanan Saya
                  </Link>
            ...
            <div className="flex ">
              <Link
                to="/auth/login"
                className="btn hidden md:flex btn-secondary btn-outline bg-secondary-content btn-sm normal-case ml-8"
              >
                Masuk
              </Link>
              <Link
                to="/auth/login"
                className="btn flex md:hidden btn-secondary mr-4 btn-sm rounded-lg normal-case ml-8"
              >
                Masuk
              </Link>
              <Link
                to="/auth/register"
                className="btn hidden md:flex btn-secondary btn-sm normal-case ml-3"
              >
                Daftar
              </Link>
            </div>
      ...
      <div className="pb-20 lg:pb-0 relative bg-base-200 bg-secondary min-h-screen">
        <Outlet context={{ categories, search }} />
      </div>
      {/* {
      ...
      <div className="btm-nav border border-base-200 fixed inset-x-0 bottom-0 z-50 bg-base-100 lg:hidden">
        ...
        <NavLink className="hover:bg-base-200" to="/customers/myorders">
